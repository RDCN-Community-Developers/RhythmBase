...
{
    public static readonly BpmCache Default = new(1, TimeSpan.Zero, DefaultBpm);
...
{
    internal readonly Level Collection;
    private BpmCache[] _bpmCache = [];
    private CpbCache[] _cpbCache = [];
    internal BeatCalculator(Level level)
    {
        Collection = level;
    }
    ...
    private void MoveEvents(int previousCpb,
                            ...
                            bool moveTrival)
    {
        RedBlackTree<TickTime, TypedEventCollection<EventType, TickTime>> allEvents = Collection.EventsBeatOrder;
        ...
        KeyValuePair<TickTime, TypedEventCollection<EventType, TickTime>>[] nodes = [.. allEvents.Where(e => e.Key.Tick >= st)];
        foreach (var node in nodes)
        {
            ...
            {
                TypedEventCollection<EventType, TickTime> existing = allEvents[newBeat];
                foreach (IBaseEvent e in node.Value)
                    existing.Add(e);
            }
            else
    ...
    /// <param name="beat">The one-based beat within the bar.</param>
    /// <returns>An <see cref="TickTime"/> bound to this calculator.</returns>
    public TickTime BeatOf(int bar, float beat) => new(this, bar, beat);
    ...
    /// <returns>The resulting interval.</returns>
    public Components.Range IntervalOf(TimeSpan timeSpan1, TimeSpan timeSpan2) => IntervalOf(BeatOf(timeSpan1), BeatOf(timeSpan2));
    ITickRange<TickTime> IBeatCalculator<TickTime>.IntervalOf(TickTime beat1, TickTime beat2) => IntervalOf(beat1, beat2);
    ITickRange<TickTime> IBeatCalculator<TickTime>.IntervalOf(TimeSpan timeSpan1, TimeSpan timeSpan2) => IntervalOf(timeSpan1, timeSpan2);
    /// <summary>
