...
internal class EventInstanceConverterSetFilterAdvanced : EventInstanceConverterBaseTaggedTileEvent<SetFilterAdvanced>
{
    protected override bool Read(ref Utf8JsonReader reader, ReadOnlySpan<byte> propertyName, ref SetFilterAdvanced value, MetadataJsonSerializerOptions options)
    {
        ...
        if (propertyName.SequenceEqual("filter"u8))
        {
            ...
            if (string.IsNullOrEmpty(filter) || !FilterTypeUtils._string2enum.TryGetValue(filter, out filterType))
                return false;
        }
        ...
        else if (propertyName.SequenceEqual("filterProperties"u8))
        {
            ...
            if (FilterTypeUtils.converters.TryGetValue(filterType, out FilterInstanceConverterBase? converter))
    ...
    protected override void Write(Utf8JsonWriter writer, ref SetFilterAdvanced value, MetadataJsonSerializerOptions options)
    {
        ...
        writer.WritePropertyName("filterProperties"u8);
        {
            ...
            if (FilterTypeUtils.converters.TryGetValue(value.FilterProperties.Type, out FilterInstanceConverterBase? converter))
