...
internal class EventEnumerator<TEvent> : EventEnumerator<TEvent, EventType, RDBeat>, IEventEnumerable<TEvent>
    where TEvent : IBaseEvent
{
    public EventEnumerator(RedBlackTree<RDBeat, TypedEventCollection<EventType, RDBeat>> source, ReadOnlyEnumCollection<EventType> types, RDRange range)
...
partial class Extensions
{
    ...
    extension<TEvent>(IEventEnumerable<TEvent> source) where TEvent : IBaseEvent
    {
        ...
        {
            RDRange merged = new RDRange(start, end).Intersect(source.Range as RDRange? ?? new RDRange(null, null));
        ...
        {
            RDRange merged = range.Intersect(source.Range as RDRange? ?? new RDRange(null, null));
            return new EventEnumerator<TEvent>(source.EventsBeatOrder, source.Types, merged);
        }
        /// <summary>
        ...
        {
            if (source is EventEnumerator<TEvent, EventType, RDBeat> casted)
                return casted.AtBeat(beat);
            if (source is OrderedEventCollection<TEvent, EventType, RDBeat> ordered)
            {
                var collection = ordered.eventsBeatOrder.TryGetValue(beat, out TypedEventCollection<EventType, RDBeat>? b) ? b : [];
    ...
    extension(IEventEnumerable<IBaseEvent> source)
    {
        ...
        {
            ReadOnlyEnumCollection<EventType> merged = Utils.EventTypeUtils.ToEnums(typeof(TEvent)).Intersect(source.Types);
            return new EventEnumerator<TEvent>(source.EventsBeatOrder, merged, source.Range as RDRange? ?? new RDRange(null, null));
        }
        ...
        public IEventEnumerable<IBaseEvent> OfEvents(params EventType[] types)
        {
            ...
            return new EventEnumerator<IBaseEvent>(source.EventsBeatOrder, merged, source.Range as RDRange? ?? new RDRange(null, null));
        }
