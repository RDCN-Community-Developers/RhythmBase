...
internal class NoIndentScope : IDisposable
{
    private readonly Utf8JsonWriter writer;
    private readonly MemoryStream stream;
    private Utf8JsonWriter inlineWriter;
    private MemoryStream inlineStream;
    private readonly RDJsonSerializerOptions options;
    public NoIndentScope(System.Text.Encodings.Web.JavaScriptEncoder? encoder, RDJsonSerializerOptions options)
    {
    ...
    public void WriteNoIndentArrayTo<T>(bool align, Utf8JsonWriter instance, IEnumerable<T> values, Action<Utf8JsonWriter, T, RDJsonSerializerOptions> writeValue)
    {
        ...
        if (align)
        {
            ...
            for (int i = 0; i < elements.Length; i++)
            {
                ...
                elements[i] = JsonElement.Parse(buffer);
                inlineWriter.Reset();
